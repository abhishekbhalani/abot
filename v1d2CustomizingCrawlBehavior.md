...
The easiest way to change Abot's behavior for common features is to change the config values that control them. See the [Quickstart](v1d2Quickstart.md) page for examples on the different ways Abot can be configured.
...
[TaskThreadManager.cs](https://github.com/sjdirect/abot/blob/master/Abot/Core/TaskThreadManager.cs) is the default IThreadManager used by Abot.
...
[MemoryManager.cs](https://github.com/sjdirect/abot/blob/master/Abot/Core/MemoryManager.cs) is the default implementation used by the crawler.
